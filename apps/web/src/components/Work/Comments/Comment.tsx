...
const Comment: FC<Props> = ({ comment }) => {
  ...
  return (
    <div className="flex items-start justify-between">
      <div className="flex items-start justify-between">
        ...
        <div className="flex flex-col items-start mr-2">
          <span className="flex items-center mb-1 space-x-1">
            ...
            {/* {checkValueInAttributes(comment?.metadata.attributes as Attribute[], 'tip') && (
              <Tooltip placement="top" content="Tipper">
                <span>
                  <HashExplorerLink
                    hash={getValueFromTraitType(comment?.metadata.attributes as Attribute[], 'hash') || ''}
                  >
          ...
          <div className={clsx('text-sm opacity-80', clamped ? 'line-clamp-2' : '')}>
            {
              // comment?.hidden ? (
              //   <span className="text-xs italic opacity-80">Comment deleted by user!</span>
              // ) : getIsImageComment() ? (
          ...
          {showMore && (
            <div className="inline-flex mt-3">
              ...
              >
                {clamped ? (
                  <>
                    Show more <ChevronDownIcon className="h-3 ml-1 w-3" />
                  </>
                ) : (
                  <>
      ...
      <div>
        {/* <ReportModal video={comment} show={showReport} setShowReport={setShowReport} />
